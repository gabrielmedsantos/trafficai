...
class TemplatesService {
    ...
    async syncFromMeta(connectionId, workspaceId) {
        ...
        if (!conn)
            throw common_types_1.HttpError.notFound('Conexão não encontrada');
        // Templates sincronizados precisam de createdBy (FK não-nullable em users).
        // Usa o primeiro ADMIN global do sistema como "dono" (sync é op do sistema, não do leadUser).
        const sysAdmin = await database_1.prisma.user.findFirst({ where: { role: 'ADMIN' }, select: { id: true } });
        if (!sysAdmin)
            throw common_types_1.HttpError.internal('Nenhum admin do sistema encontrado para registrar como dono do template');
        ...
        for (const mt of metaTemplates) {
            ...
            // Busca escopada por conexão — evita "adotar" templates órfãos (connectionId=NULL)
            // ou de outras conexões que coincidam metaTemplateId/nome.
            ...
            if (existing) {
                ...
                // Usa formato nested (connection.connect) porque createdBy também usa relation nested.
                await database_1.prisma.template.create({
                    data: {
                        ...
                        createdBy: { connect: { id: sysAdmin.id } },
                    },
                });
                created++;
            }
